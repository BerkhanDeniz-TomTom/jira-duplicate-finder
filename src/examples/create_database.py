...
def main():
    ...
    # Example JQL filter
    jql_filter = ('project in (Navigation, "Audi HCP3") AND '
                  'issuetype = Bug AND '
                  '(Customer in (Audi_HCP3, Audi_OCI) OR Products in (Audi_HCP3, Audi_OCI)) AND '
                  '("External Reference" is not EMPTY OR "Customer ID" is not EMPTY) AND '
